...
class TestConnectionPool:
    async def test_str(self):
        ...
        assert str(connection_pool) ==\
            "<ConnectionPool host=localhost port=11211 total_connections=0>"
        assert repr(connection_pool) ==\
            "<ConnectionPool host=localhost port=11211 total_connections=0>"
    ...
    async def test_connection_context_one_create_connection(self, event_loop, mocker):
        ...
        create_protocol = mocker.patch(
            "fastcache.connection_pool.create_protocol",
            CoroutineMock(return_value=Mock())
        )
    ...
    async def test_waiters_cancellation_is_supported(self, event_loop, mocker):
        ...
        # check that where called in the right order
        assert waiters_woken_up == \
            [connection_context3, connection_context1]
