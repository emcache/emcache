...
class TestNodesSelection:
    def test_one_node(self):
        ...
        node = object()
        rendezvous_node = cyfastcache.RendezvousNode("host", 11211, node)
        keys_per_node = cyfastcache.nodes_selection([b"foo"], [rendezvous_node])
    ...
    def test_three_nodes(self):
        ...
        keys_per_node = cyfastcache.nodes_selection(keys, list_of_rendezvous_nodes)
        ...
        # Operations for the same keys should be idempotent when same
        # keys are used.
        next_round_keys_per_node = cyfastcache.nodes_selection(keys, list_of_rendezvous_nodes)
